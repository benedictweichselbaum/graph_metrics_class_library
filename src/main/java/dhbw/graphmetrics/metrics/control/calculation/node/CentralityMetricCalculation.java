...
public final class CentralityMetricCalculation {
    ...
    /**
     * Method that calculates the closeness centrality of a node in a disconnected graph
     *
     * @param graph graph to be analyzed
     * @param node  node to be analyzed
     * @param <N>   type of node
     ...
     * @return closeness centrality of node in graph
     */
    public static <N extends Comparable<N>, E> Double closenessCentralityDisconnected(Graph<N, E> graph, N node) {
        int distanceSum = 0;
        int reachable = 0;
        for (Integer distance : SearchAlgorithms.advancedBreadthFirstSearch(graph, node).getFirstObject().values()) {
            if (distance == Integer.MAX_VALUE || distance == 0) continue;

            reachable++;
            distanceSum += distance;
        }
        if (distanceSum == 0) return 0.0;
        return (Math.pow((double) reachable / (double) (graph.nodes().size() - 1), 2) * (1.0 / (double) distanceSum));
    }
